...
#include "../../../Source/Compression/Csc/CscCompressor.h"
...
#if 0
  class DummyOutputStream : public Nuclex::Storage::Binary::OutputStream {

    public: bool CanAcceptMoreData() const override { return true; }

    public: void WriteUpTo(
...
namespace Nuclex { namespace Storage { namespace Compression { namespace Csc {
  ...
  TEST(CscCompressorTest, CanBeCreatedAndDestroyed) {
    EXPECT_NO_THROW(
      CscCompressor compressor;
      (void)compressor;
    );
  }
  ...
  TEST(CscCompressorTest, MemoryChunkCanBeCompressed) {

    // Message that will be compressed
    char message[] = u8"Hello World, this text will be compressed by CSC";
  ...
  TEST(CscCompressorTest, MemoryChunkSurvivesCompressionRoundTrip) {

    // Message that will be compressed
    char message[] = u8"This text will be compressed and then decompressed again by CSC";
    const std::uint8_t *input = reinterpret_cast<const std::uint8_t *>(message);
