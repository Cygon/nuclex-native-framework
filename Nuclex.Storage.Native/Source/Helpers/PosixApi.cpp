...
namespace Nuclex { namespace Storage { namespace Helpers {
...
#if defined(NUCLEX_STORAGE_STRERROR_IS_THREAD_LOCAL)
  ...
  bool PosixApi::IsPathError(int errorNumber) {
    return (
      (errorNumber == ELOOP) ||
      (errorNumber == EMLINK) ||
  ...
  bool PosixApi::GetEnvironmentVariable(const std::string &name, std::string &value) {
  ...
  void PosixApi::ThrowExceptionForSystemError(const std::string &errorMessage, int errorNumber) {
    using Nuclex::Storage::Helpers::PosixApi;
    ...
    } else if(PosixApi::IsPathError(errorNumber)) {
